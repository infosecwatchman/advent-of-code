    ...
    for line in inputdata.split('\n'):
        for character in line:
            if not character.isdigit() and character != ".":
                characterset.update(character)
    print(characterset)
    previousline = ""
    nextline = ""
    currentline = ""
    for linenum, line in enumerate(inputdata.split('\n')):
        currentline = line
        if linenum != 0:
            previousline = inputdata.split('\n')[linenum-1]
        if linenum != len(inputdata.split('\n'))-1:
            nextline = inputdata.split('\n')[linenum+1]
        line.find(di)
