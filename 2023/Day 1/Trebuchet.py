...
for number in numbers:
    spellednumbers.append(inflector.number_to_words(number))
...
for line in input.split('\n'):
    ...
    while should_restart:
        ...
        holdnumber = ""
        holdnumberlocation = 0
        holdlocation = 1000
        for numnum, number in enumerate(spellednumbers):
            if line.find(number) != -1 and line.find(number) < holdlocation:
                holdlocation = line.find(number)
                holdnumber = number
                holdnumberlocation = numnum
        if holdlocation != 1000:
            line = line.replace(holdnumber, str(holdnumberlocation)+holdnumber[-1], 1)
            should_restart = True
    trueinput = trueinput + "\n" + line

def calculateLaunch(input):
    answerreg = re.findall(".*?([0-9]).*(\d)(?!.*\d)|.*([0-9])", input)
    answer = 0
    for set in answerreg:
        if (set[2] == ''):
            pairnum = int(str(set[0]) + str(set[1]))
            answer = answer+pairnum
        if (set[0] == ''):
            z = int(str(set[2]) + str(set[2]))
            answer = answer+z
    return answer

print("Part 1:", calculateLaunch(input))
print("Part 2:", calculateLaunch(trueinput))